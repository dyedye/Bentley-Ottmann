...
class AvlBiTree(BiTree):
    ...
    def remove(self, key):
        ...
        """
        print(f'removed fkey {key}')
        node = self.lookup(key)
        if not node:
            raise ValueError("tree doesn't have element '{}'".format(key))
            return False
    ...
    def search_lower(self, key, lower=-float('inf')):
        node = self.root
        if node.key is None:
            return None
        ans = lower
    ...
    def search_higher(self, key, higher=float('inf')):
        node = self.root
        if node.key is None:
            return higher
